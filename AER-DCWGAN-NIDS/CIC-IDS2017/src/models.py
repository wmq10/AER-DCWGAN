...
class ConditionalDiscriminator(nn.Module):
    def __init__(self, latent_dim, img_shape, num_classes):
        ...
        self.label_emb = nn.Embedding(num_classes, latent_dim)  #
        self.net = nn.Sequential(
            spectral_norm(nn.Linear(latent_dim + np.prod(img_shape) + latent_dim, 100)),  #
            nn.LeakyReLU(0.2),
    ...
    def forward(self, z, img, labels):
        ...
        combined = torch.cat([z, img_flat, label_emb], dim=1)  #
...
def compute_gradient_penalty(model, real_imgs, fake_imgs, noise, labels):
    ...
    d_interpolates = model.discriminator(noise.detach(), interpolates_img, labels)  #
    ...
    return model.gp_weight * gradient_penalty  #
class AE_WGAN_AER(nn.Module):
    def __init__(self, img_shape, num_classes, latent_dim=32, recon_weight=1.5, gp_weight=3, aer_weight=2):
        ...
        self.optimizer_E = optim.Adam(self.encoder.parameters(), lr=1e-5, betas=(0.5, 0.9))
        self.optimizer_G = optim.Adam(self.generator.parameters(), lr=1e-5, betas=(0.5, 0.9))
        self.optimizer_D = optim.Adam(self.discriminator.parameters(), lr=1e-5, betas=(0.5, 0.9))


        self.encoder_discriminator = EncoderDiscriminator(latent_dim).to(device)
        self.optimizer_ED = optim.Adam(self.encoder_discriminator.parameters(), lr=1e-5, betas=(0.5, 0.9))
    ...
    def train(self, attack_name, reshaped_X_attack, labels, epochs=EPOCHS, batch_size=BATCH_SIZE):
        ...
        dataloader = DataLoader(dataset, batch_size=batch_size, sampler=sampler, drop_last= False)
        metrics = {
            'epoch': [],
            'd_loss': [],
            'g_loss': [],
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                ...
                if d_total > 0:
                    real_mean = D_real_total / d_total  #
                    fake_mean = D_fake_total / d_total
                    wasserstein = real_mean - fake_mean
                else:
                    wasserstein = 0.0
                ...
                if len(dataloader) > 0:
                    ...
                    current_recon_loss = 0.0
                current_ed_loss = epoch_ed_loss / len(dataloader) if len(dataloader) > 0 else 0.0
...
if __name__ == "__main__":
    ...
    mock_labels = np.random.randint(0, 5, 1000)
