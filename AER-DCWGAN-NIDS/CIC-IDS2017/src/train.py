    ...
    for attack_idx, attack_name in enumerate(all_attacks):
        ...
        if existing_samples < 200:
            if len(np.unique(y_attack)) == 1:
                print(f"样本数不足 ({existing_samples} < 200)，但只有一个类别，无法使用 SMOTE，复制样本至200条")
                # 复制样本
                ...
                print(f"复制后样本数: {existing_samples}")
            else:
                print(f"样本数不足 ({existing_samples} < 200)，应用 SMOTE 增强至200条")

                # 应用 SMOTE
                smote = SMOTE(random_state=42, k_neighbors=min(5, existing_samples - 1))
                X_attack, y_attack = smote.fit_resample(X_attack, y_attack)
                existing_samples = len(X_attack)
                print(f"SMOTE 增强后样本数: {existing_samples}")

        if attack_data.shape[1] != 31:  # 24特征 + 1标签
        ...
        # 重塑为模型输入形状
        try:
            reshaped_X = X_scaled.reshape(-1, 1, 5, 6)  # 调整为合适的形状
        ...
        # 根据样本数量动态设置生成数量（修复变量未定义问题）
        if existing_samples > 80000:
            print(f"跳过训练: {attack_name} 样本充足（{existing_samples} > 100000）")
        else:
        ...
        if num_to_generate > 0:
            ...
            # 初始化模型，传递正确的类别数（假设CICIDS2017有12个类别）
            ae_wgan = AE_WGAN_AER(
                img_shape=(1, 5, 6),
                num_classes=12,
                latent_dim=32
            ...
            generated_denorm = generated_samples.reshape(-1, 30)  # 调整为24个特征
        else:
