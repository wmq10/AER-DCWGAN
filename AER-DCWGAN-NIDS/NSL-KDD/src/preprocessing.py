...
def preprocess_nslkdd(raw_data_dir, dataset_split_dir):
    ...
    joblib.dump(label_to_num, LABEL_MAP_PATH)  # 保存标签到数值的映射
    ...
    def create_feature_pipeline():
        return Pipeline([
            ...
            ('variance', VarianceThreshold(threshold=0.05)),  # 过滤低方差特征
            ('rfe', RFE(estimator=RandomForestClassifier(random_state=42), n_features_to_select=24)),  # 特征选择
            ('scaler', MinMaxScaler(feature_range=(-1, 1)))  # 归一化应在特征选择后
        ])
    ...
    # 提取并保存 MinMaxScaler（此时pipeline已定义）
    minmax_scaler = pipeline.named_steps['scaler']
    joblib.dump(minmax_scaler, SCALER_PATH)  # 仅保存Scaler
    ...
    print(f"RFE特征下基准模型准确率: {base_accuracy}")
...
def _split_and_save(df, dest_dir, prefix):
    for label_name in df['Label'].unique():
        ...
        if not subset.empty:
            ...
            print(f"警告: {prefix[:-1]}数据集中无{label_name}数据")
